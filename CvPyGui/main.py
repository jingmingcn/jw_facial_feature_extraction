...
model = load_model('emotion_model.hdf5')  # Update this path to your model file
            ...
            while self._win1_running:
                    ...
                    if ret:
                        ...
                        for (x, y, w, h) in faces:
                            ...
                            # Predict the emotion
                            emotion_probabilities = model.predict(preprocessed_face)[0]
                            max_index = np.argmax(emotion_probabilities)
                            emotion_label = emotion_labels[max_index]
                            
                            # f.write(str(id) + "\t" + emotion_label + "\n")
                            emotion_frame = frame.copy()
                            height, width , _ = emotion_frame.shape
                            dst_width = self.voice_image.size().width()
                            dst_height = self.voice_image.size().height()
                            r = dst_width / width if dst_width / width < dst_height / height else dst_height / height

                            emotion_frame = imutils.resize(emotion_frame, width=dst_width)

                            x, y, w, h = int(x*r), int(y*r), int(w*r), int(h*r)

                            # Draw a rectangle around the face and put the emotion label
                            cv2.rectangle(emotion_frame, (x, y), (x+w, y+h), (255, 0, 0), 2)
                            emotion_frame = cv2PutText(emotion_frame, emotion_label, (x, y-int(10*r)), (36,255,12), 30)
                            self.voice_image.updateImage(emotion_frame)

                        # detect faces in the grayscale image
                        rects = detector(gray, 0)
                        landmark_frame = frame.copy()
                        whiteFrame = frame.copy()
                        whiteFrame.fill(0)
                        ...
                        for (i, rect) in enumerate(rects):
                            ...
                            for (x, y) in shape:
                                cv2.circle(landmark_frame, (x, y), 2, (0, 255, 0), -1)
                                cv2.circle(whiteFrame, (x, y), 2, (0, 255, 0), -1)
                            
                            self.eye_image.updateImage(landmark_frame)
                            self.original2_image.updateImage(whiteFrame)
